...
module.exports = function(grunt) {
            ...
            if (typeof(specs) === 'string') {
                ...
                // All strings containing non-alpha characters are direct file patterns.
                    ...
                    for (i=0; i < src.length; i++) {
                        var skip = false;
                        if (ignoreDirs) {
                            for (k=0; k < ignoreDirs.length; k++) {
                                if (src[i].substr(0, ignoreDirs[k].length) === ignoreDirs[k]) {
                                    skip = true;
                                    break;
                                }
                            }
                        }
                        if (!skip && !grunt.file.isDir(src[i])) {
     ...
     */
    function fontFiles() {
        return files(cog.getConfig('media.fonts'), 'fonts');
    }
    ...
    /**
     * Find all data files (not models) to include in HTML.
     */
    function srcDataFiles() {
     ...
     */
    function codeFiles() {
        return removeDuplicates(files(cog.getConfig('src.code'), 'code'), configFiles().concat(libFiles()).concat(modelFiles()).concat(srcDataFiles()).concat(extLibFiles()));
    }
    ...
    /**
     * Find all source code that is (potentially) written in Typescript.
     */
     function srcTypescriptFiles() {
         return srcFiles();
     }
     ...
     */
    function mediaFiles() {
        return picFiles().concat(soundFiles()).concat(fontFiles()).concat(otherMediaFiles());
    }
    ...
    function distCssFiles() {
        var src = cog.pathDist() + cog.getConfig('name') + '.css';
        var subdir = cog.getConfig('src.css') || '';
        if (subdir && subdir.indexOf('*') >= 0) {
            subdir = subdir.substr(0, subdir.indexOf('*'));
        }
        var dst = cog.pathDist() + subdir + cog.getConfig('name') + '.min.css';
    ...
    function includeUnitTestJsFiles() {
        var libs = removeDuplicates(extLibFiles().concat(unitTestLibraryFiles())).concat(generatedJsFiles());
        if (cog.getOption('include_only_external')) {
            return libs;
        }
    ...
    function generatedJsFiles(what) {
        ...
        if (cog.getOption('compile_typescript')) {
            ...
            for (var i = 0; i < src.length; i++) {
                if (src[i].src.substr(-3) === '.ts') {
                    src[i].dst = src[i].src.substr(0, src[i].src.length - 3) + '.js';
                    ret.push(src[i]);
                }
            }
        }
    ...
    /**
     * List of localization data files.
     */
    function localizationDataFiles() {
        return files(cog.getConfig('data.localization'));
    }

    /**
     * Data files that are text and are only needed for development.
     */
    function developmentTextDataFiles() {
        return files(cog.getConfig('data.devel.text'));
    }

    /**
     * Other non text data files that are only needed for development.
     */
    function developmentOtherDataFiles() {
        return files(cog.getConfig('data.devel.other'));
    }

    /**
     * All data files that are text.
     */
    function textDataFiles() {
        return developmentTextDataFiles().concat(localizationDataFiles());
    }
     ...
     */
    function dataFiles() {
        return textDataFiles().concat(developmentOtherDataFiles());
    }
    ...
    function fileCategoryMap() {
        ...
        var categories = ['extLibFiles', 'extLibMapFiles', 'extCssFiles', 'extFontFiles', 'fontFiles',
            'appIndexFiles', 'testIndexFiles', 'configFiles', 'libFiles', 'modelFiles', 'srcDataFiles',
            ...
            'pythonFiles', 'compiledPythonFiles', 'localizationDataFiles', 'developmentOtherDataFiles', 'developmentTextDataFiles'];
    ...
    return {
        ...
        localizationDataFiles: localizationDataFiles,
        developmentOtherDataFiles: developmentOtherDataFiles,
        developmentTextDataFiles: developmentTextDataFiles,
        textDataFiles: textDataFiles,
        dataFiles: dataFiles,
        srcTypescriptFiles: srcTypescriptFiles,
    };
};
