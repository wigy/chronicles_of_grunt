...
module.exports = function(grunt) {
    ...
    function getOption(name) {
        ...
        if (ret === undefined) {
            ...
            if (frameworks.options[framework]) {
                ret = frameworks.options[framework][name];
            }
        }
        return ret;
    }

    // TODO: Move all configuration related stuff above to the separate module.
            ...
            if (typeof(specs) === 'string') {
                ...
                for (j=0; j < src.length; j++) {
                    ...
                    if (dstPrefix.substr(0, 7) === 'CONFIG:') {
                        dstPrefix = getConfig(dstPrefix.substr(7), 'lib');
                    }
     ...
     */
    function codeFiles() {
        return removeDuplicates(files(getConfig('src.code'), 'code'), configFiles().concat(libFiles()).concat(modelFiles()).concat(dataFiles()).concat(extLibFiles()));
    }
     ...
     */
     ...
     function otherNonJsFiles() {
         return removeDuplicates(files(getConfig('src.other'), 'other'), srcFiles().concat(otherJsFiles().concat(appIndexFiles())));
     }
    ...
    function otherMediaFiles() {
        return files(getConfig('media.other'), 'other');
    }
    ...
    function includeJsFiles() {
        if (getOption('include_only_external')) {
    ...
    function unitTestHelperFiles() {
        return files(getConfig('test.unit.helpers'), 'test');
    }
    ...
    function allJavascriptFiles() {
        return srcFiles().concat(otherJsFiles()).concat(ff.unitTestFiles()).concat(ff.unitTestHelperFiles()).concat(taskFiles()).concat(commonJsFiles());
    }
    ...
    function pythonFiles() {
        return files(getConfig('src.python'));
    }
    ...
    function compiledPythonFiles() {
        return files(getConfig('compiled.python'));
    }
