...
module.exports = {
    ...
    'Task `dist`': function(test) {
        ...
        runner.run('libs');
        runner.run('dist');
        var index = runner.read('output/distro/index.html');
        ...
        var css = runner.read('output/distro/cog_unittest.min.css');
        test.ok(css.indexOf('color') > 0, "does not have color definition in cog_unittest.min.css");
        var js = runner.read('output/distro/cog_unittest.min.js');
